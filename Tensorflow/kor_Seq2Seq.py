...
class Seq2Seq(object):
    def __init__(self, xseq_len, yseq_len,
        ...
        def __graph__():
            ...
            basic_cell=  tf.contrib.rnn.rnn_cell.BasicLSTMCell(emb_dim, state_is_tuple=True)
            basic_cell = tf.contrib.rnn.rnn_cell.DropoutWrapper(basic_cell, output_keep_prob=self.keep_prob)
            # stack cells together : n layered model
            stacked_lstm = tf.contrib.rnn.rnn_cell.MultiRNNCell([basic_cell] * num_layers, state_is_tuple=True)
            # for parameter sharing between training model
            #  and testing model
            with tf.variable_scope('decoder') as scope:
                # build the seq2seq model
                #  inputs : encoder, decoder inputs, LSTM cell type, vocabulary sizes, embedding dimensions
                self.decode_outputs, self.decode_states = tf.nn.seq2seq.embedding_rnn_seq2seq(self.enc_ip,
                                                                                                          self.dec_ip,
                                                                                                          stacked_lstm,
                                                                                                          xvocab_size,
                                                                                                          yvocab_size,
            ...
            self.loss = tf.nn.seq2seq.sequence_loss(self.decode_outputs, self.labels, loss_weights,
                                                                yvocab_size)
        ...
        for i in range(self.epochs):
            try:
                ...
                if i and i % (self.epochs // 100) == 0:  # TODO : make this tunable by the user
                    ...
                    #print("saver 됌")
                    val_loss = self.eval_batches(sess, valid_set, 30)  # TODO : and this
                    # print stats
                    #print("여기는 안됌")
